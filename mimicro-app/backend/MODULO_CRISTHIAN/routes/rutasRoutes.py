...
@router.post("")
async def route_crear_ruta(request: Request, usuario=Depends(get_current_user)):
    requiere_rol("admin")(usuario)
    body = await request.json()
    return await crear_ruta(body)


@router.put("/{ruta_id}")
async def route_actualizar_ruta(ruta_id: int, request: Request, usuario=Depends(get_current_user)):
    requiere_rol("admin")(usuario)
    body = await request.json()
    return await actualizar_ruta(ruta_id, body)


@router.delete("/{ruta_id}")
async def route_desactivar_ruta(ruta_id: int, usuario=Depends(get_current_user)):
    requiere_rol("admin")(usuario)
    return await desactivar_ruta(ruta_id)
